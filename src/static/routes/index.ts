...
모던 웹 서비스에서 Flex나 Grid를 적극적으로 이용한 다수의 레이아웃 케이스들을 개발자 도구를 통해 확인하였고,
이를 이해하고 활용해야 모던 웹에서의 기본기를 갖출 수 있다고 판단했습니다.

또한 CSS 가상 클래스에 대한 이해도도 높이고 싶었던 와중이었기에 
이에 가장 잘 어울리는 프로젝트는 "웹 사이트 클론"이라고 생각했고
위와 같은 의도로 웹 페이지 클로닝을 진행했습니다.
`,
...
"유지 보수 가능한 형태의 코드란 무엇인가"에 대한 간접적인 경험을 습득할 수 있었습니다.`,
  ...
  },
  {
    ...
    details: {
      ...
      purpose: `첫 팀 프로젝트이자 JavaScript 및 CSS 전처리기에 대한 이해도를 높일 수 있었던 프로젝트입니다.
...
- 사용자 검색 내용 로컬 스토리지에 저장 및 삭제 모듈 구현
- 사용자 검색 정보 유효성 검사 기능 구현
...
이는 업무의 지속성을 확보된다는 것을 체감했습니다.
또한 전처리기/후처리기에 대한 개념과 사용법을 학습해 생산성을 확보했으며,
...
API 요청에 따른 이미 만들어져있는 데이터를 반환받아서 개발을 진행하는 데에도 큰 매력을 느꼈습니다.
그렇기 때문에, JS 역치를 더 향상시켜보고자 하는 의도로
다시 한 번 Node.js로 서버를 구현하고 문서를 서빙하는 개인 프로젝트를 진행했습니다.

해당 개인 프로젝트는 "개인 만족"이라는 키워드에 집중해서 평소에 좋아하는 무언가를 만들어보고 싶었고
어릴 때 자주 보고 좋아했던 만화인 "나루토"의 API를 이용해서 반환 데이터를 통한 간단한 나루토 캐릭터 카드를 만들어보고자 하는 의도로서 진행됐습니다.
`,
...
Node.js의 가능성 또한 확인할 수 있었기 때문에 저의 성장 가능성에 있어서 조금씩 자리잡고있던 자그마한 의심들이 사라졌습니다.`,
    },
  },
  {
    id: 9,
...
그럼에도 예상치 못한 데이터가 반환될 수도 있고, 꼭 프로미스 체이닝이 아닐지라도 단순 문자열과 같은 체이닝들에 대해서도
...
개발자에게 불편함이란 곧, 아이디어로 연결된다는 사실을 깨닫게 되는 계기가 되는 프로젝트였습니다.
`,
    },
  },
  {
...
`,
      ...
      report: `서로 다른 기준과 스타일을 이해하며 진행할 수 있었던 좋은 경험이였습니다.

5인 기준의 팀 단위 작업의 템포를 이해 및 체감할 수 있어서 똑같은 상황이 주어졌을 때
더 좋은 결과를 기대할 수 있다는 생각이 들었습니다.

특히 맡은 바 작업을 진행했던 DB 기능 개발에 있어서, CRUD는 오히려 기본기이며
...
또 한가지 중요한 점은, 프레임워크나 라이브러리 사용에 대한 부담감이 사라졌다는 것을 체감했습니다.
NPM도 생소했던 시기를 생각해보면 Vanilla JS 코딩을 통해 역치를 뚫어놓은 덕을 봤다는 것을 새삼스레 깨닫게 되었습니다.
`,
    },
  },
  {
    ...
    details: {
      ...
      purpose: `해커톤 지정 과제로서 저출산, 고령화에 필요한 서비스를 개발해야했습니다.
팀 회의를 통해 위 주제를 파고들 수 있는 핵심을 "노동"으로 정했고
팀원 각자의 직장 생활 경험을 상기하며
"노동"환경에서부터 영향력을 확보해 저출산, 고령화에 문제에
확보된 간접적 영향력을 행사하는 서비스를 개발하려는 의도로 진행했습니다.
`,
      mainTask: `- 아이디어 수립
- 관련 기사(통계) 스크랩
...
`,
      ...
      report: `비록 해커톤에서 떨어져서 코딩은 하지 못 했지만,
...
`,
  ...
  },
  {
    ...
    details: {
      ...
      purpose: `개인 프로젝트에 사용할 유튜브 데이터들을 수집하기 위해 개발했습니다.
`,
...
"크롤러"라는 단어가 가지는 왜인지 모를 무게감에 조금은 긴장감이 있었는데 구현하고 나니 연기처럼 사라진 느낌이 듭니다.
체감 난이도와는 다르게 수집된 다량의 데이터는 저에게는 큰 선물처럼 느껴졌고 크롤러가 가진 강력함을 체감할 수 있었습니다.
`,
    },
  },
  {
    ...
    details: {
      ...
      purpose: `야생 동물을 분류할 수 있게 훈련시킨 모델을 이용해
  생태 통로 모니터링 및 농가 & 주민을 대상으로 피해 방지 시스템을 구현하기 위한 의도로 개발을 진행했습니다.
`,
...
그렇지만 늘 욕심을 내는 만큼 나의 것으로 만들 수 있다고 생각하는 사람이라 그런건지는 몰라도
실제 개발을 진행할 때에는 긴장감이 꽤 많이 수그러들었습니다.
긴장감이 수그러든 구체적인 이유를 혼자서 생각하다보니 모델을 훈련하고 훈련한 모델의 동작을 예상하는 그 과정 자체를 생각보다 많이 즐겼던 것 같습니다.

크롤러에게 "데이터"를 선물 받았었다고 한다면, 훈련된 모델에게는 그보다 깊은 "충성도"를 선물받은 느낌이였습니다.

언제 또 이런 시기에 이런 경험과 감정을 가질 수 있을지는 미지수지만 매우 귀한 경험이였습니다.
`,
    },
  },
...
채널의 메타 데이터나 동영상의 상세 정보 같은 유틸리티 컨텐츠들을 제공하기 위한 의도로 개발을 진행했습니다.
`,
...
`,
      ...
      report: `욕심나는 무엇이든 내 것으로 만들 수 있다는 것이 이 분야의 가장 큰 매력임을 다시금 깨달았던 프로젝트입니다.

이 프로젝트를 진행하면서
"지금 내가 이 기술이 필요한가? 적용을 고려해야 하나?" 혹은
"꼭 이 라이브러리를 사용해서 구현해야 하나?"와 같은 개발자로서의 이해타산적 시야가 넓어진 것이 체감됐기 때문에
그동안 만들어온 것들이 무의미하지 않았다는 것을 다시 상기시킬 수 있었습니다.
`,
    },
  },
];
